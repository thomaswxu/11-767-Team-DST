...
class Module(nn.Module):
    ...
    @torch.no_grad() # added by team dst
    def run_pred(self, dev, args=None, name='dev', iter=0):
        ...
        # store latency info for lab
        avg_latency = 0
        num_passes = 0
        for batch, feat in self.iterate(dev, args.batch):
            
            # forward pass with timing calc
            num_passes += 1
            start = timer()
            out = self.forward(feat)
            end = timer()
            print("FORWARD PASS LATENCY: " + str((end-start)))
            print("AVG FORWARD PASS LATENCY (after " + str(num_passes) + " passes): " + str(avg_latency*((num_passes-1)/num_passes) + (1/num_passes)*(end - start)) + " s") # Time in seconds, e.g. 5.38091952400282
            avg_latency = avg_latency*((num_passes-1)/num_passes) + (1/num_passes)*(end - start)
            ...
            print(f"losses for 1 epoch: {loss}")
    ...
    def run_eval_custom(self, splits, args=None, optimizer=None):
        valid_seen = splits['valid_seen']
        valid_unseen = splits['valid_unseen']
        ...
        # debugging: chose a small fraction of the dataset
        dataset_fraction = 400 # 100 corresponds to 15 samples
        # small_train_size = int(dataset_fraction * 0.7)
        print(f"size of valid_seen: {len(valid_seen)}")
        print(f"size of valid_unseen: {len(valid_unseen)}")

        small_valid_size = int((dataset_fraction * 0.3) / 2)
        small_valid_size = 2
        # train = model.train[:small_train_size]
        # valid_seen = valid_seen[:small_valid_size]
        # valid_unseen = valid_unseen[:small_valid_size]

        # compute metrics for valid_seen
        p_valid_seen, valid_seen_iter, total_valid_seen_loss, m_valid_seen = self.run_pred(valid_seen, args=args, name='valid_seen')
        m_valid_seen.update(self.compute_metric(p_valid_seen, valid_seen))
        m_valid_seen['total_loss'] = float(total_valid_seen_loss)
        print(('valid_seen/total_loss: ' + str(m_valid_seen['total_loss']) + str(valid_seen_iter)))

        # compute metrics for valid_unseen
        p_valid_unseen, valid_unseen_iter, total_valid_unseen_loss, m_valid_unseen = self.run_pred(valid_unseen, args=args, name='valid_unseen')
        m_valid_unseen.update(self.compute_metric(p_valid_unseen, valid_unseen))
        m_valid_unseen['total_loss'] = float(total_valid_unseen_loss)
        print('valid_unseen/total_loss: ' + str(m_valid_unseen['total_loss']) + str(valid_unseen_iter))
    ...
    def load_task_json(self, task):
        ...
        '''
        self.args.data = "data/json_feat_2.1.0"

        json_path = os.path.join(self.args.data, task['task'], '%s' % self.args.pp_folder, 'ann_%d.json' % task['repeat_idx'])
        # print(f"data path: {self.args.data}")
        # print(f"tasks[tasks]: {task['task']}")
        # print(f"task json path: {json_path}")
